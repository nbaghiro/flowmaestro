 ...
 * Uses a level-based hierarchical layout with special handling for conditional branches
 *
 * This is a pure algorithm with no browser/React dependencies - can be used in both
...
export const ENTRY_NODE_TYPES = ["trigger", "input", "files", "url", "audioInput"];
 ...
 *
 * @param nodes - Array of nodes with id, type, and current position
 ...
 * @param edges - Array of edges with source, target, and optional sourceHandle
 * @param entryNodeId - The ID of the entry point node
 * @returns Map of node IDs to their positions
 */
export function autoLayoutNodes(
    ...
    const adjacencyList = buildSimpleAdjacencyList(nodes, edges);

    // Assign levels to nodes using BFS
    const levels = assignLevelsBFS(entryNodeId, adjacencyList);
...
/**
 * Build simple adjacency list from edges (for BFS-based layout)
 */
function buildSimpleAdjacencyList(nodes: LayoutNode[], edges: LayoutEdge[]): Map<string, string[]> {
    const adjacencyList = new Map<string, string[]>();
...
/**
 * Assign level (depth) to each node using BFS (simpler, for generated workflows)
 */
function assignLevelsBFS(
    entryNodeId: string,
    adjacencyList: Map<string, string[]>
): Map<string, number> {
    const levels = new Map<string, number>();
...
/**
 * Group nodes by their level (simple version for BFS-based layout)
 */
function groupNodesByLevelSimple(
    nodes: LayoutNode[],
    levels: Map<string, number>
...
/**
 * Input format for workflow nodes from AI generation
 */
...
/**
 * Input format for workflow edges from AI generation
 */
...
/**
 * Convert generated workflow to React Flow format with auto-calculated positions
 *
 * @param generatedNodes - Nodes from AI workflow generation
 * @param generatedEdges - Edges from AI workflow generation
 * @param entryNodeId - The ID of the entry point node

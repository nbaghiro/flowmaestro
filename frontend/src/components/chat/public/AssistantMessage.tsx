...
/**
 * Check if a URL or filename represents media content
 */
function isMediaAttachment(url: string, fileName?: string): boolean {
    const checkString = (url + (fileName || "")).toLowerCase();
    ...
    return mediaExtensions.some((ext) => checkString.includes(ext));
}
                ...
                {message.attachments && message.attachments.length > 0 && (
                    <div className="mt-2 space-y-2">
                        {message.attachments.map((attachment, index) =>
                            isMediaAttachment(attachment.url, attachment.fileName) ? (
                                <MediaPreview
                                    key={index}
                                    src={attachment.url}

...
import type { FormInterfaceSummary } from "@flowmaestro/shared";
import { Badge } from "../common/Badge";

export interface FormInterfaceCardProps {
    formInterface: FormInterfaceSummary;
    isSelected?: boolean;
...
export function FormInterfaceCard({
    formInterface: fi,
    isSelected = false,
    onClick,
    onContextMenu,
        ...
        >
            ...
            <div className="relative">
                <div
                    ...
                    style={{
                        backgroundColor: fi.coverType === "color" ? fi.coverValue : "#6366f1",
                        backgroundImage:
            ...
            <div className={`p-4 ${fi.iconUrl ? "pt-8" : ""}`}>
                <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-foreground truncate">{fi.title}</h3>
                        {fi.description && (
                        ...
                        >
                            ...
                            {isMenuOpen && (
                                <div className="absolute right-0 mt-1 w-48 bg-card border border-border rounded-lg shadow-lg py-1 z-50">
                                    ...
                                    {onViewLive && fi.status === "published" && (
                                        <button
                                            onClick={() => {
                                                setIsMenuOpen(false);
                                                onViewLive();
                                    ...
                                    {onViewSubmissions && (
                                        ...
                                        >
                                            ...
                                            Submissions ({fi.submissionCount ?? 0})
                                        </button>
                                    )}
                                    {onDuplicate && (
                                        <button
                ...
                <div className="flex items-center gap-2 mt-3">
                    <Badge variant={fi.status === "published" ? "success" : "default"}>
                        {fi.status === "published" ? (
                            <>
                    ...
                    <span className="text-xs text-muted-foreground">
                        {fi.submissionCount ?? 0} submissions
                    </span>
                </div>

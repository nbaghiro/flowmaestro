...
export function streamAgentExecution(
    ...
    eventSource.addEventListener("completed", (event) => {
        try {
            ...
            if (data.executionId === executionId) {
                ...
                // Close connection to prevent auto-reconnect loop
    ...
    eventSource.addEventListener("error", (event) => {
        try {
            ...
            // Generic error - don't close here as this might be a parse error
...
export function streamWorkflowExecution(
    ...
    // Track if we intentionally closed the connection (to avoid error callback on terminal events)
    let intentionallyClosed = false;

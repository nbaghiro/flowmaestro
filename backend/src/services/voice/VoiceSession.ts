...
import { DeepgramStreamClient, ElevenLabsStreamClient } from "@flowmaestro/ai-sdk";
import type {
    VoiceSessionState,
    VoiceClientMessage,
...
export class VoiceSession {
    ...
    async initialize(): Promise<void> {
        try {
            // Initialize Deepgram STT client using SDK
            this.deepgram = new DeepgramStreamClient({
                apiKey: config.ai.deepgram.apiKey,
                deepgram: {
            ...
            // Initialize ElevenLabs TTS client using SDK with agent's voice config
            this.elevenlabs = new ElevenLabsStreamClient({
                apiKey: config.ai.elevenlabs.apiKey,
                elevenlabs: {
                    voiceId: agentVoiceConfig?.voiceId || config.voice.elevenlabs.defaultVoiceId,
                    modelId: (agentVoiceConfig?.model || config.voice.elevenlabs.model) as
                        | "eleven_turbo_v2_5"
                        | "eleven_multilingual_v2"
                        | "eleven_flash_v2_5",
                    stability: agentVoiceConfig?.stability ?? config.voice.elevenlabs.stability,
                    similarityBoost:
                        agentVoiceConfig?.similarityBoost ??
                        config.voice.elevenlabs.similarityBoost,

/**
 * Sage Provider Unit Tests
 *
 * Tests the SageProvider class in isolation.
...
describe("SageProvider", () => {
    let provider: SageProvider;

    beforeEach(() => {
        provider = new SageProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("sage");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "sage".replace(/-/g, "_") + "_";

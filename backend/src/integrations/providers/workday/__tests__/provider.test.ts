/**
 * Workday Provider Unit Tests
 *
 * Tests the WorkdayProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { WorkdayProvider } from "../WorkdayProvider";
...
describe("WorkdayProvider", () => {
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "workday".replace(/-/g, "_") + "_";

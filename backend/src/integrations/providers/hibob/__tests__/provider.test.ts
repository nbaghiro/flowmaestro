/**
 * Hibob Provider Unit Tests
 *
 * Tests the HiBobProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { HiBobProvider } from "../HiBobProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("HiBobProvider", () => {
    let provider: HiBobProvider;

    beforeEach(() => {
        provider = new HiBobProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("hibob");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "hibob".replace(/-/g, "_") + "_";

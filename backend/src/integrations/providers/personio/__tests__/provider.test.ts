/**
 * Personio Provider Unit Tests
 *
 * Tests the PersonioProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { PersonioProvider } from "../PersonioProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("PersonioProvider", () => {
    let provider: PersonioProvider;

    beforeEach(() => {
        provider = new PersonioProvider();
    });
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "personio".replace(/-/g, "_") + "_";

/**
 * Bamboohr Provider Unit Tests
 *
 * Tests the BambooHRProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { BambooHRProvider } from "../BambooHRProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("BambooHRProvider", () => {
    let provider: BambooHRProvider;

    beforeEach(() => {
        provider = new BambooHRProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("bamboohr");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "bamboohr".replace(/-/g, "_") + "_";

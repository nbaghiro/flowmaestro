/**
 * AmazonSellerCentral Provider Unit Tests
 *
 * Tests the AmazonSellerCentralProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { AmazonSellerCentralProvider } from "../AmazonSellerCentralProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("AmazonSellerCentralProvider", () => {
    let provider: AmazonSellerCentralProvider;

    beforeEach(() => {
        provider = new AmazonSellerCentralProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("amazon-seller-central");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "amazon-seller-central".replace(/-/g, "_") + "_";

/**
 * Etsy Provider Unit Tests
 *
 * Tests the EtsyProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { EtsyProvider } from "../EtsyProvider";
...
describe("EtsyProvider", () => {
    ...
    beforeEach(() => {
        provider = new EtsyProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("etsy");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "etsy".replace(/-/g, "_") + "_";

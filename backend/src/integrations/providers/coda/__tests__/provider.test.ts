/**
 * Coda Provider Unit Tests
 *
 * Tests the CodaProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { CodaProvider } from "../CodaProvider";
...
describe("CodaProvider", () => {
    ...
    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("coda");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "coda".replace(/-/g, "_") + "_";

/**
 * Miro Provider Unit Tests
 *
 * Tests the MiroProvider class in isolation.
...
describe("MiroProvider", () => {
    let provider: MiroProvider;

    beforeEach(() => {
        provider = new MiroProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("miro");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "miro".replace(/-/g, "_") + "_";

/**
 * Hootsuite Provider Unit Tests
 *
 * Tests the HootsuiteProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { HootsuiteProvider } from "../HootsuiteProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("HootsuiteProvider", () => {
    let provider: HootsuiteProvider;

    beforeEach(() => {
        provider = new HootsuiteProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("hootsuite");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "hootsuite".replace(/-/g, "_") + "_";

/**
 * GoogleDocs Provider Unit Tests
 *
 * Tests the GoogleDocsProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { GoogleDocsProvider } from "../GoogleDocsProvider";
...
describe("GoogleDocsProvider", () => {
    ...
    beforeEach(() => {
        provider = new GoogleDocsProvider();
    });
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "google-docs".replace(/-/g, "_") + "_";

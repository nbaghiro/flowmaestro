...
/**
 * MySQL Provider - implements database operations via connection pooling
 */
export class MysqlProvider extends BaseProvider {
     ...
     */
    private createMysqlPool(connection: ConnectionWithData): Pool {
        if (!isDatabaseConnectionData(connection.data)) {
            throw new Error("Invalid connection data for MySQL");
        }
        ...
        if (dbData.connection_string) {
            return createPool({
                ...
                connectionLimit: dbData.max_connections || 10,
        ...
        if (!dbData.host || !dbData.database || !dbData.username) {
            throw new Error(
                "MySQL connection requires either connection_string or (host, database, username)"
            );
        }
    ...
    private getOrCreatePool(connection: ConnectionWithData): Pool {
        ...
        const pool = this.createMysqlPool(connection);

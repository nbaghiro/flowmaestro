/**
 * Evernote Provider Unit Tests
 *
 * Tests the EvernoteProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { EvernoteProvider } from "../EvernoteProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("EvernoteProvider", () => {
    let provider: EvernoteProvider;

    beforeEach(() => {
        provider = new EvernoteProvider();
    });
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "evernote".replace(/-/g, "_") + "_";

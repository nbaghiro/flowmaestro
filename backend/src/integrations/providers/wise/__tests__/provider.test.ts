 ...
 *
 * Tests the WiseProvider class in isolation.
...
describe("WiseProvider", () => {
    let provider: WiseProvider;

    beforeEach(() => {
        provider = new WiseProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("wise");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "wise".replace(/-/g, "_") + "_";

/**
 * Typeform Provider Unit Tests
 *
 * Tests the TypeformProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { TypeformProvider } from "../TypeformProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("TypeformProvider", () => {
    let provider: TypeformProvider;

    beforeEach(() => {
        provider = new TypeformProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("typeform");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "typeform".replace(/-/g, "_") + "_";

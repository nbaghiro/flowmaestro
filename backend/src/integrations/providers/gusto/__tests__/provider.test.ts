/**
 * Gusto Provider Unit Tests
 *
 * Tests the GustoProvider class in isolation.
...
describe("GustoProvider", () => {
    let provider: GustoProvider;

    beforeEach(() => {
        provider = new GustoProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("gusto");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "gusto".replace(/-/g, "_") + "_";

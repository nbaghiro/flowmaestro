/**
 * Confluence Provider Unit Tests
 *
 * Tests the ConfluenceProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { ConfluenceProvider } from "../ConfluenceProvider";
...
describe("ConfluenceProvider", () => {
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "confluence".replace(/-/g, "_") + "_";

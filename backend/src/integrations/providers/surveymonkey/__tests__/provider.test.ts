/**
 * Surveymonkey Provider Unit Tests
 *
 * Tests the SurveyMonkeyProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { SurveyMonkeyProvider } from "../SurveyMonkeyProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("SurveyMonkeyProvider", () => {
    let provider: SurveyMonkeyProvider;

    beforeEach(() => {
        provider = new SurveyMonkeyProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("surveymonkey");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "surveymonkey".replace(/-/g, "_") + "_";

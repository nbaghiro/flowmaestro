/**
 * Netsuite Provider Unit Tests
 *
 * Tests the NetsuiteProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { NetsuiteProvider } from "../NetsuiteProvider";
...
describe("NetsuiteProvider", () => {
    ...
    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("netsuite");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "netsuite".replace(/-/g, "_") + "_";

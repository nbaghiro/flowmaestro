/**
 * Rippling Provider Unit Tests
 *
 * Tests the RipplingProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { RipplingProvider } from "../RipplingProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("RipplingProvider", () => {
    let provider: RipplingProvider;

    beforeEach(() => {
        provider = new RipplingProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("rippling");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "rippling".replace(/-/g, "_") + "_";

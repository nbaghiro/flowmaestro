/**
 * MicrosoftExcel Provider Unit Tests
 *
 * Tests the MicrosoftExcelProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { MicrosoftExcelProvider } from "../MicrosoftExcelProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("MicrosoftExcelProvider", () => {
    let provider: MicrosoftExcelProvider;

    beforeEach(() => {
        provider = new MicrosoftExcelProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("microsoft-excel");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "microsoft-excel".replace(/-/g, "_") + "_";

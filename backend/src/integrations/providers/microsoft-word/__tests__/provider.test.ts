/**
 * MicrosoftWord Provider Unit Tests
 *
 * Tests the MicrosoftWordProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { MicrosoftWordProvider } from "../MicrosoftWordProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("MicrosoftWordProvider", () => {
    let provider: MicrosoftWordProvider;

    beforeEach(() => {
        provider = new MicrosoftWordProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("microsoft-word");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "microsoft-word".replace(/-/g, "_") + "_";

/**
 * Expensify Provider Unit Tests
 *
 * Tests the ExpensifyProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { ExpensifyProvider } from "../ExpensifyProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("ExpensifyProvider", () => {
    let provider: ExpensifyProvider;

    beforeEach(() => {
        provider = new ExpensifyProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("expensify");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "expensify".replace(/-/g, "_") + "_";

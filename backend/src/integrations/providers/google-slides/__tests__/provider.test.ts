/**
 * GoogleSlides Provider Unit Tests
 *
 * Tests the GoogleSlidesProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { GoogleSlidesProvider } from "../GoogleSlidesProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("GoogleSlidesProvider", () => {
    let provider: GoogleSlidesProvider;

    beforeEach(() => {
        provider = new GoogleSlidesProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("google-slides");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "google-slides".replace(/-/g, "_") + "_";

/**
 * MicrosoftOutlook Provider Unit Tests
 *
 * Tests the MicrosoftOutlookProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { MicrosoftOutlookProvider } from "../MicrosoftOutlookProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("MicrosoftOutlookProvider", () => {
    let provider: MicrosoftOutlookProvider;

    beforeEach(() => {
        provider = new MicrosoftOutlookProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("microsoft-outlook");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "microsoft-outlook".replace(/-/g, "_") + "_";

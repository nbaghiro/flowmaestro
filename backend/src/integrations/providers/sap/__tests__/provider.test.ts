/**
 * Sap Provider Unit Tests
 *
 * Tests the SapProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { SapProvider } from "../SapProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("SapProvider", () => {
    let provider: SapProvider;
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "sap".replace(/-/g, "_") + "_";

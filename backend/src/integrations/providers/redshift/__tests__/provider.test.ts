/**
 * Redshift Provider Unit Tests
 *
 * Tests the RedshiftProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { RedshiftProvider } from "../RedshiftProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("RedshiftProvider", () => {
    let provider: RedshiftProvider;

    beforeEach(() => {
        provider = new RedshiftProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("redshift");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "redshift".replace(/-/g, "_") + "_";

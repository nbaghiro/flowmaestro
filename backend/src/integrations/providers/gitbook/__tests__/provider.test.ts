/**
 * Gitbook Provider Unit Tests
 *
 * Tests the GitBookProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { GitBookProvider } from "../GitBookProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("GitBookProvider", () => {
    let provider: GitBookProvider;

    beforeEach(() => {
        provider = new GitBookProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("gitbook");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "gitbook".replace(/-/g, "_") + "_";

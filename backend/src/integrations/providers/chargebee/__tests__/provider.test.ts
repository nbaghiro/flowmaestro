/**
 * Chargebee Provider Unit Tests
 *
 * Tests the ChargebeeProvider class in isolation.
...
describe("ChargebeeProvider", () => {
    let provider: ChargebeeProvider;

    beforeEach(() => {
        provider = new ChargebeeProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("chargebee");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "chargebee".replace(/-/g, "_") + "_";

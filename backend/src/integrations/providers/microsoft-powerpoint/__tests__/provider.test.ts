/**
 * MicrosoftPowerpoint Provider Unit Tests
 *
 * Tests the MicrosoftPowerPointProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { MicrosoftPowerPointProvider } from "../MicrosoftPowerPointProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("MicrosoftPowerPointProvider", () => {
    let provider: MicrosoftPowerPointProvider;

    beforeEach(() => {
        provider = new MicrosoftPowerPointProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("microsoft-powerpoint");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "microsoft-powerpoint".replace(/-/g, "_") + "_";

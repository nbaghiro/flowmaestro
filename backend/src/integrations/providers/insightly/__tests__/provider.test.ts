/**
 * Insightly Provider Unit Tests
 *
 * Tests the InsightlyProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { InsightlyProvider } from "../InsightlyProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("InsightlyProvider", () => {
    let provider: InsightlyProvider;

    beforeEach(() => {
        provider = new InsightlyProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("insightly");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "insightly".replace(/-/g, "_") + "_";

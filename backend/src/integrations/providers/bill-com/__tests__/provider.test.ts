/**
 * BillCom Provider Unit Tests
 *
 * Tests the BillComProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { BillComProvider } from "../BillComProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("BillComProvider", () => {
    let provider: BillComProvider;

    beforeEach(() => {
        provider = new BillComProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("bill-com");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "bill-com".replace(/-/g, "_") + "_";

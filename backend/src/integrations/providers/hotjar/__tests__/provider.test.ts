/**
 * Hotjar Provider Unit Tests
 *
 * Tests the HotjarProvider class in isolation.
...
describe("HotjarProvider", () => {
    let provider: HotjarProvider;

    beforeEach(() => {
        provider = new HotjarProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("hotjar");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "hotjar".replace(/-/g, "_") + "_";

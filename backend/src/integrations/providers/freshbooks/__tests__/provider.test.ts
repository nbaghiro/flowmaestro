/**
 * Freshbooks Provider Unit Tests
 *
 * Tests the FreshBooksProvider class in isolation.
 * Validates operation registration, trigger setup, auth config, and webhook configuration.
 */

import { FreshBooksProvider } from "../FreshBooksProvider";
import type { OAuthConfig, WebhookConfig } from "../../../core/types";

describe("FreshBooksProvider", () => {
    let provider: FreshBooksProvider;
    ...
    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("freshbooks");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "freshbooks".replace(/-/g, "_") + "_";

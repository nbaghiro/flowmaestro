/**
 * Tableau Provider Unit Tests
 *
 * Tests the TableauProvider class in isolation.
...
describe("TableauProvider", () => {
    let provider: TableauProvider;

    beforeEach(() => {
        provider = new TableauProvider();
    });

    describe("provider metadata", () => {
        it("has correct name and display name", () => {
            expect(provider.name).toBe("tableau");
    ...
    describe("MCP tools", () => {
        ...
        it("follows naming convention", () => {
            const tools = provider.getMCPTools();
            const expectedPrefix = "tableau".replace(/-/g, "_") + "_";

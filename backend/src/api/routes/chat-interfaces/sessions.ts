...
export async function listChatInterfaceSessionsRoute(fastify: FastifyInstance) {
    ...
    fastify.get("/:id/sessions", async (request, reply) => {
        ...
        try {
            // Check if chat interface exists and belongs to workspace
            ...
            logger.error({ id, workspaceId, error }, "Error listing chat interface sessions");
    ...
    fastify.get("/:id/sessions/:sessionId", async (request, reply) => {
        const chatInterfaceRepo = new ChatInterfaceRepository();
        const sessionRepo = new ChatInterfaceSessionRepository();
        ...
        try {
            ...
            logger.error(
                { id, sessionId, workspaceId, error },
    ...
    fastify.get("/:id/stats", async (request, reply) => {
        ...
        try {
            ...
            logger.error({ id, workspaceId, error }, "Error getting chat interface stats");
            return reply.status(500).send({
                success: false,
                error: error instanceof Error ? error.message : String(error)
            });

    ...
    fastify.post(
        ...
        async (request, reply) => {
            ...
            try {
                ...
                const agent = await agentRepo.findByIdAndUserId(body.agentId, userId);
                if (!agent) {
                    return reply.status(400).send({
                        success: false,
                ...
                const isAvailable = await chatInterfaceRepo.isSlugAvailable(body.slug, userId);
                if (!isAvailable) {
                    return reply.status(400).send({
                        success: false,
                ...
                const chatInterface = await chatInterfaceRepo.create(userId, body);

                logger.info(
                    { chatInterfaceId: chatInterface.id, userId },
                ...
                logger.error({ userId, body, error }, "Error creating chat interface");

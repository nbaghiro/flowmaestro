...
export async function uploadChatInterfaceAssetRoute(fastify: FastifyInstance) {
    fastify.post("/:id/assets", async (request, reply) => {
        ...
        try {
            ...
            logger.info(
                { chatInterfaceId: id, assetType, workspaceId },
            ...
            logger.error({ id, workspaceId, error }, "Error uploading chat interface asset");

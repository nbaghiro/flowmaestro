        ...
        async (request, reply) => {
            ...
            try {
                ...
                if (body.slug !== undefined) {
                    ...
                    const isAvailable = await chatInterfaceRepo.isSlugAvailable(
                        body.slug,
                        userId,
                        id
                    );
                ...
                const chatInterface = await chatInterfaceRepo.update(id, userId, body);
                ...
                logger.info({ chatInterfaceId: id, userId }, "Chat interface updated");
                ...
                logger.error({ id, userId, error }, "Error updating chat interface");
                return reply.status(500).send({
                    success: false,

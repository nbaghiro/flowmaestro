...
export async function publicFormInterfaceQueryRoutes(fastify: FastifyInstance) {
     ...
     */
    fastify.post("/:slug/submissions/:submissionId/query", async (request, reply) => {
        ...
        try {
            ...
            try {
                const embeddingResult = await embeddingService.generateEmbeddings([body.query], {
            ...
            try {
                ...
                logger.error({ slug, submissionId, error: searchError }, "Failed to search chunks");
            ...
            logger.error({ slug, submissionId, error }, "Error querying form interface submission");
            return reply.status(500).send({
                success: false,

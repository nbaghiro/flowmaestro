 ...
 * Wait Node Execution
 *
 * Complete execution logic and handler for wait/delay nodes.
 * Supports duration-based delays and timestamp-based waiting.
 */
...
/**
 * Handler for Wait node type.
 */
export class WaitNodeHandler extends BaseNodeHandler {
    readonly name = "WaitNodeHandler";
    readonly supportedNodeTypes = ["wait", "delay", "sleep"] as const;

    async execute(input: NodeHandlerInput): Promise<NodeHandlerOutput> {
        const startTime = Date.now();
        const context = getExecutionContext(input.context);

        const result = await executeWaitNode(input.nodeConfig, context);

        // Extract wait duration for signals if needed
        const waitResult = result as unknown as WaitNodeResult;

        return this.success(
            result,
            {
                // Signal the wait duration to the workflow engine
                waitDurationMs: waitResult.actualWaitDuration
            },
            {
...
/**
 * Factory function for creating Wait handler.
 */
export function createWaitNodeHandler(): WaitNodeHandler {

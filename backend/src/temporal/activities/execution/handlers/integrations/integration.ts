 ...
 *
 * Complete execution logic and handler for generic integration nodes.
 * Uses the Provider SDK system to execute operations on various services.
 */
...
/**
 * Handler for Integration node type.
 */
export class IntegrationNodeHandler extends BaseNodeHandler {
    readonly name = "IntegrationNodeHandler";
    readonly supportedNodeTypes = ["integration"] as const;
    ...
    async execute(input: NodeHandlerInput): Promise<NodeHandlerOutput> {
        ...
        const result = await executeIntegrationNode(
            input.nodeConfig as unknown as IntegrationNodeConfig,
            context
        );
...
/**
 * Factory function for creating Integration handler.
 */
export function createIntegrationNodeHandler(): IntegrationNodeHandler {
    return new IntegrationNodeHandler();
}

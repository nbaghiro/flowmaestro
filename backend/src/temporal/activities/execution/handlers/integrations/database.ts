/**
 * Database Node Execution
 *
 * Complete execution logic and handler for database operation nodes.
 * Supports PostgreSQL, MySQL, and MongoDB via the provider system.
 */
...
/**
 * Handler for Database node type.
 */
export class DatabaseNodeHandler extends BaseNodeHandler {
    readonly name = "DatabaseNodeHandler";
    readonly supportedNodeTypes = ["database"] as const;
    ...
    async execute(input: NodeHandlerInput): Promise<NodeHandlerOutput> {
        ...
        const result = await executeDatabaseNode(
            input.nodeConfig as unknown as DatabaseNodeConfig,
            context
        );
...
/**
 * Factory function for creating Database handler.
 */
export function createDatabaseNodeHandler(): DatabaseNodeHandler {
    return new DatabaseNodeHandler();
}

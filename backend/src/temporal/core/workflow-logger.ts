 ...
 * Workflow Logger
 *
 * Console-based logging for Temporal workflows running in V8 sandbox.
 * This file must NOT import any Node.js modules (pino, fs, etc.)
 *
...
const LOG_LEVEL_PRIORITY: Record<WorkflowLogLevel, number> = {
    debug: 0,
    info: 1,
    warn: 2,
    error: 3
...
const currentLogLevel: WorkflowLogLevel = "info";

function shouldLog(level: WorkflowLogLevel): boolean {
...
export function createWorkflowLogger(context: WorkflowLogContext): WorkflowLogger {
    ...
    const logMethod = (level: WorkflowLogLevel) => {
        return (
            message: string,

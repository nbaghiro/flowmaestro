...
// Simulate sending to a channel
function simulateChannelSend(
    channel: string,
...
async function simulateNotificationPipeline(
    ...
    for (const channel of channelsForPriority) {
        ...
        const result = simulateChannelSend(channel, request, shouldFail, latency);
    ...
    describe("output summary", () => {
        ...
        it("should include message IDs for delivered channels", async () => {
            ...
            result.channelResults
                .filter((r) => r.success)
                .forEach((r) => {
                    expect(r.messageId).toMatch(new RegExp(`^${r.channel}_msg_`));
                });
        });
    });

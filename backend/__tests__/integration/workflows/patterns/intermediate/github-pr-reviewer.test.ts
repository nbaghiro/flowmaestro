...
describe("GitHub PR Reviewer Pattern", () => {
    ...
    describe("sandbox data integration", () => {
        it("should have GitHub fixtures available", () => {
            expect(hasSandboxFixture("github", "createIssue")).toBe(true);
        });
        ...
        it("should have Jira fixtures available for ticket creation", () => {
            expect(hasSandboxFixture("jira", "createIssue")).toBe(true);
        });

        it("should have Linear fixtures available as Jira alternative", () => {
            expect(hasSandboxFixture("linear", "createIssue")).toBe(true);
        });

        it("should use sandbox data for Slack notification action", async () => {
        ...
        it("should use sandbox data for Jira ticket creation", async () => {
            const jiraOutput = await createSandboxActionOutput("jira", "createIssue", {
                projectKey: "SEC",
                summary: "Critical security issue in PR #123",
                issueType: "Bug"
            });

            expect(jiraOutput.success).toBe(true);
            expect(jiraOutput.fromSandbox).toBe(true);
        });

        it("should support custom error scenarios for GitHub API failures", async () => {
        ...
        it("should integrate sandbox actions into pattern execution", async () => {
            // Create mocks that use sandbox data for integration nodes
            const slackAction = await createSandboxActionOutput("slack", "sendMessage", {
                channel: "#code-reviews",
                text: "Review complete for PR #123"
            });
            ...
            const result = await simulatePatternExecution({
                ...
                inputs: { prEvent: samplePREvent },
                mockOutputs: createCompleteMocks({
                    "action-slack-alert": slackAction
                })
            });
        ...
        it("should list available providers from fixture registry", () => {
            const fixtureRegistry = getFixtureRegistry();
            const providers = fixtureRegistry.getProviders();

            // Verify key providers needed for PR review workflow are available

...
/**
 * Get sandbox response for notification integration
 * Uses real Slack fixtures when available for rejection notifications
 */
async function getSandboxNotificationResponse(
    requestId: string,
    status: string
): Promise<{ notified: boolean; requestId: string; status: string; notifiedAt: number } | null> {
    // Try to get sandbox response for Slack sendMessage (for rejection notifications)
...
async function simulateApprovalWorkflow(
    ...
    buildApprovalWorkflow(request.approvers.length);
    ...
    if (path === "approved") {
        ...
        // Try sandbox notification first
        const sandboxNotification = await getSandboxNotificationResponse(
            request.requestId,
            finalStatus
        );
        const notificationOutput = sandboxNotification || {
            notified: true,
            requestId: request.requestId,
            status: finalStatus,
            notifiedAt: Date.now()
        };
        context = storeNodeOutput(context, "NotifyRejection", notificationOutput);
    }
...
describe("Multi-Step Approval Workflow", () => {
    ...
    describe("sandbox data integration", () => {
        it("should verify Slack fixtures are available for notifications", () => {
            expect(fixtureRegistry.has("slack", "sendMessage")).toBe(true);
        });

        it("should use sandbox data for rejection notifications", async () => {
            const approvers = createApprovers(1);
            const request: ApprovalRequest = {
                requestId: "sandbox_test_001",
                requestType: "expense",
                requesterId: "user_sandbox",
                requesterName: "Sandbox Test User",
                amount: 1000,
            ...
            decisions.set("approver_1", {
                ...
                decision: "rejected",
                comments: "Testing sandbox integration",
                timestamp: Date.now()
            });
            ...
            const notifyOutput = result.context.nodeOutputs.get("NotifyRejection") as {
                notified: boolean;
                status: string;
            };
            expect(notifyOutput.notified).toBe(true);
            expect(notifyOutput.status).toBe("rejected");
        });

        it("should register custom notification error scenarios", async () => {
            // Register a custom error scenario for notification failure
            sandboxDataService.registerScenario({
                id: "notification-service-down",
                provider: "slack",
                operation: "sendMessage",
                paramMatchers: { channel: "#down" },
                response: {
                    success: false,
                    error: {
                        type: "server_error",
        ...
        it("should support multiple provider fixtures for complex workflows", () => {
            // Verify multiple providers are available for complex approval workflows
            const providers = fixtureRegistry.getProviders();
            expect(providers).toContain("slack");
            expect(providers).toContain("hubspot");
            // These could be used for CRM updates, email notifications, etc.
        });
    });
